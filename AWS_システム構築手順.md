...
|項目|設定値|説明|
|---|---|---|
|ターゲットグループ名|uda_target_group|このターゲットグループの名前。自分が管理するための任意の名前を付ける。|
...
ターゲットグループの一覧で先ほど作成した[uda_target_group]をチェックし、  
ページ上部の[アクション]→[インスタンス/IPターゲットの登録と登録解除]をクリック。  

表示された画面下部のEC2インスタンス一覧に、  
以前作成した[uda_web_server_nginx]と[uda_web_server_nginx_2]があると思うので2つをチェックし[登録済みに追加]をクリックします。  
...
|項目|設定値|説明|
|---|---|---|
|名前|uda_alb|このALBの名前。自分が管理するための任意の名前を付ける。|
...
[ターゲットグループ]の項目で先ほど作成した[uda_target_group]を選択してください。
...
ロードバランサーの一覧で先ほど作成した[uda_alb]をチェックし、ページ下部に表示される詳細情報にある[DNS名]を確認します。
